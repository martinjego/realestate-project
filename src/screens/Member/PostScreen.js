...
class PostScreen extends Component {
  ...
  componentDidMount() {
    ...
    listing_actions.get_all_list(user_id, api_token); 
  }
  onClickEdit(list_id) {
  ...
  getListing() {
    return this.props.listings.all_list.map(list => {
      var swipeoutBtns = [
        { 
          text: 'Edit', 
          onPress: () => this.onClickEdit(list.id),
          component: (
      ...
      return (
        <Swipeout style={styles.swipeoutStyle} key={list.id} right={swipeoutBtns}>
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <MemberHeader title="Posts" sort={() => navigate('DrawerOpen')}/>
        <ScrollView>
